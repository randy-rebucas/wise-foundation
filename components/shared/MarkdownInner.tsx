...
const baseComponents: Components = {
  h1: ({ children }) => (
    <h1 className="mt-0 mb-3 text-xl font-semibold tracking-tight text-foreground">{children}</h1>
  ),
  h2: ({ children }) => (
  ...
  ul: ({ children }) => (
    <ul className="mb-3 ml-4 list-disc space-y-1.5 text-sm text-muted-foreground">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="mb-3 ml-4 list-decimal space-y-1.5 text-sm text-muted-foreground">{children}</ol>
  ),
  ...
  pre: ({ children }) => (
    <pre className="mb-4 overflow-x-auto rounded-lg border bg-muted/40 p-3 text-xs">{children}</pre>
  ),
...
export type MarkdownInnerProps = {
  content: string;
  className?: string;
};

export function MarkdownInner({ content, className }: MarkdownInnerProps) {
  return (
